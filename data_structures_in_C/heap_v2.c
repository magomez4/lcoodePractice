...
minHeap* createHeap()
{
    ...
    newHeap->heap = (int*)malloc(INITIALHEAPSIZE*sizeof(int));
...
void heapifyUp(minHeap* obj)
{
    ...
    while(parentIdx >= 0 
      && (obj->heap[i] < obj->heap[parentIdx]))
    {
        swap(obj, i, parentIdx);
        i = parentIdx;
        parentIdx = floor((i-1)/2);
    }
}
...
void heapifyDown(minHeap* obj, int idx)
{
    ...
    // if we didn't start heapifying from 0, then
    // we need to heapify again from 0 to make sure
    // we didn't break the heap property while swaping
    if(idx != 0)
    {
        heapifyDown(obj, 0);
    }
...
void ensureHeapCapacity(minHeap* obj, int newSize)
{
    if(obj->capacity <= newSize)
    {
        ...
        obj->heap = (int*)realloc(obj->heap, (2*obj->capacity)*sizeof(int));
        if(obj->heap == NULL)
        {
            printf("BAD ALLOCATION\n");
            free(obj);
...
void addToHeap(minHeap* obj, int val)
{
...
int findInHeap(minHeap* obj, int val)
{
...
void removeFromHeap(minHeap* obj, int val)
{
...
int main() {
    ...
    while(numQueries > 0)
    {
        ...
        long int function = strtol(string, &endPointer, 10);
